...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        infoButton = (FloatingActionButton) findViewById(R.id.infoBtn);

        infoButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                String url = "https://github.com/Qwatrum/BikeOutlook";
    ...
    private Marker generateMarker(int id, int i, String name, double lat, double lon) {
        ...
        marker.setOnMarkerClickListener(new Marker.OnMarkerClickListener() {
            ...
            public boolean onMarkerClick(Marker marker, MapView mapView) {
                marker.setIcon(getDrawable(R.drawable.baseline_directions_bike_24_highlight));
    ...
    private void prepareRequest(int i, int id, String name, GeoPoint stationPos) {
        Request request = new Request.Builder()
                .url(requestURL+String.valueOf(id))
                .build();
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                try (Response res = response) {
                    ...
                    if (body.contains("[-") || body.contains("[404]")) {
                        Log.e(TAG, "Unexpected code " + res);
                    } else {
                        ...
                        float [] data = {Float.parseFloat(String.valueOf(s[0])), Float.parseFloat(String.valueOf(s[1])), Float.parseFloat(String.valueOf(s[2])), Float.parseFloat(String.valueOf(s[3])), Float.parseFloat(String.valueOf(s[4])), Float.parseFloat(String.valueOf(s[5])), Float.parseFloat(String.valueOf(s[6])), Float.parseFloat(String.valueOf(s[7])), Float.parseFloat(String.valueOf(s[8]))};
    ...
    private void requestSuccesful(float [] data, int id, String name, GeoPoint stationPos) {

        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                String[] one = getPrognose(data[1], data[2]);
                String[] two = getPrognose(data[3], data[4]);
                String[] three = getPrognose(data[5], data[6]);
                String[] six = getPrognose(data[7], data[8]);
                String amountOne = one[0];
                String amountTwo = two[0];
                String amountThree = three[0];
                String amountSix = six[0];
                ...
                showBottomDialog(name, id, Math.round(data[0]), amountOne, amountTwo, amountThree, amountSix, p1, p2, p3, p6);
    ...
    private String[] getPrognose(float data1, float data2) {
        String[] res = {"",""};

        if (Math.abs((data1 - data2)) < 0.16) {
            res[0] = "~";
            res[1] = "~";
        } else {
            if (data1 > data2) {
                res[0] = "x";
                res[1] = String.valueOf(Math.round(data1*100)) + "%";
            } else {
                res[0] = "✓";
                res[1] = String.valueOf(Math.round(data2*100)) + "%";
            }
        }
    ...
    private void showBottomDialog(String name, int id, int current, String amountOne, String amountTwo, String amountThree, String amountSix, String p1, String p2, String p3, String p6) {
        ...
        TextView trendSixText = dialog.findViewById(R.id.trendSix);
        ...
        TextView percentageSixText = dialog.findViewById(R.id.percentage_six);
        ...
        trendOneText.setText(amountOne);
        trendTwoText.setText(amountTwo);
        trendThreeText.setText(amountThree);
        trendSixText.setText(amountSix);

        percentageOneText.setText(p1);
        percentageTwoText.setText(p2);
        percentageThreeText.setText(p3);
        percentageSixText.setText(p6);
