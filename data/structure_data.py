...
fieldnames = ["in_45", "in_30", "in_15", "month_sin", "month_cos", "day_sin", "day_cos", "weekend", "weather", "hour_sin", "hour_cos"]
data = []
current_station = 0
...
for i, line in enumerate(lines[3:-3]):
    ...
    for j, station in enumerate(line[3:]):
        new_data.append(int(lines[i-2+3][j+3]))
        new_data.append(int(lines[i-1+3][j+3]))
        new_data.append(int(station))
    
    data.append(new_data)

#print(data)

with open(f"train{current_station}.csv", "w") as file:
